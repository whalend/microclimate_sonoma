...
ACF <- acf(na.omit(r4_ANN[1,]), lag.max=24*365*2, plot=F)
ACF$lag <- ACF$lag/3600/24
plot(ACF, xlab="Lag (days)", main=paste("Autocorrelation, station", station.ids_ANN[1], "\n(2004/2005)"))
...
PACF <- pacf(na.omit(r4_ANN[1,]), lag.max=24*10, plot=F)
PACF$lag <- PACF$lag/3600/24
plot(PACF, xlab="Lag (days)", main=paste("Partial Autocorrelation, station", station.ids_ANN[1], "\n(2004/2005)"))
...
CROSSacf <- acf(na.omit(as(r4_ANN[rn, ], "xts")), lag.max=24*365*2, plot=F)
CROSSacf$lag <- CROSSacf$lag/3600/24
plot(CROSSacf, xlab="Lag (days)")
