...
# hours at dessicating, slow/no growth temperatures, > 20 C
rs_select_temps <- left_join(rs_select_temps, rainy_season_temps %>% 
                                  filter(temp > 20) %>%
                                  group_by(plotid, sample_year) %>%
...
write.csv(rs_select_temps, "~/GitHub/superspreaders/analysis/data/rs_select_temps.csv")
```
...
summer_season_temps <- long_temp %>% 
      mutate(sample_year = ifelse(
            date >= "2003-06-01" & date <= "2003-09-30", 2003, 
            ifelse(date >= "2004-06-01" & date <= "2004-09-30", 2004,
              ifelse(date >= "2005-06-01" & date <= "2005-09-30", 2005,
                ifelse(date >= "2006-06-01" & date <= "2006-09-30", 2006,
                  ifelse(date >= "2007-06-01" & date <= "2007-09-30", 2007,
                    ifelse(date >= "2008-06-01" & date <= "2008-09-30", 2008,
                      ifelse(date >= "2009-06-01" & date <= "2009-09-30", 2009,
        ifelse(date >= "2010-06-01" & date <= "2010-09-30", 2010, 
          ifelse(date >= "2011-06-01" & date <= "2011-09-30", 2011,
            ifelse(date >= "2012-06-01" & date <= "2012-09-30", 2012,
              ifelse(date >= "2013-06-01" & date <= "2013-09-30", 2013, NA)
)))))))))))
...
# hours at cool, 'persisting' temperatures, <= 15 C (~59 F)
summer_select_temps <- left_join(
      summer_select_temps, summer_season_temps %>%
            filter(temp <= 18) %>% 
            group_by(plotid, sample_year) %>%
            summarise(hrs_blw18 = length(temp)),
      by = c("plotid", "sample_year"))
...
write.csv(summer_select_temps, "~/GitHub/superspreaders/analysis/data/summer_select_temps.csv")
```
